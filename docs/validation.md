...
The supplied video was replayed through the locally built multilingual `small` Whisper runtime and the frozen classifier on this M4 Pro. It emitted multiple transcript updates, recovered both the positive and duplicate-charge/frustration cues, and met the latency gate after warm-up:

| Measure | Result | Gate |
| --- | ---: | ---: |
| Text classifier p95 | 7.6 ms | <=100 ms |
| Audio-to-dashboard p50 | 256 ms | <=1,000 ms |
| Audio-to-dashboard p95 | 274 ms | <=1,750 ms |
