...
The supplied video was replayed through the locally built English `medium.en` Whisper runtime and the frozen classifier on this M4 Pro. It emitted 89 provisional transcript updates, recovered both the positive and duplicate-charge/frustration cues, and met the latency gate after warm-up:

| Measure | Result | Gate |
| --- | ---: | ---: |
| Text classifier p95 | 13.9 ms | <=100 ms |
| Audio-to-dashboard p50 | 542 ms | <=1,000 ms |
| Audio-to-dashboard p95 | 660 ms | <=1,750 ms |
